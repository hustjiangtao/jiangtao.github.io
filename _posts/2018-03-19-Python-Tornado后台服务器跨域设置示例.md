...
origin = self.request.headers.get('Origin', '')  Tornado获取请求'origin'的方法
self.set_header('Access-Control-Allow-Origin', origin or '*')  设置允许的'origin'，只设置'*'时某些特定情况下会失败故最好优先获取请求的域加入允许组中
self.set_header('Access-Control-Allow-Credentials', 'true')  设置是否允许客户端携带证书式访问。通过对 Credentials 参数的设置，就可以保持跨域 Ajax 时的 Cookie
self.set_header(
...
if origin in allow_origin:
    self.set_header('Access-Control-Allow-Origin', origin)  设置允许的'origin'
self.set_header('Access-Control-Allow-Credentials', 'true')  设置是否允许客户端携带证书式访问。通过对 Credentials 参数的设置，就可以保持跨域 Ajax 时的 Cookie
self.set_header(
    'Access-Control-Allow-Headers',
    'Origin, X-Requested-With, Content-Type, Accept, client_id, uuid, Authorization'
